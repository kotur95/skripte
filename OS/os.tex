...
%            NASLOV                         PODNASLOV                  AUTOR ...
	...
	\date{\vspace{-2mm}\small{\texttt{\color{blue!50!cyan!70!black}{18.02.2018.}}}}
...
\subsection{Komandna linija}

Komandna linija predstavlja mehanizam za interakciju sa operativnim sistemom ili nekim drugim programom kojim možemo obavljati različite zadatke u okviru našeg sistema. Koncept interfejsa komandne linije je nastao još 50-ih godina 20 veka kada je ovakav način zadavanja komandi (posredstvom \texttt{tty-a} \footnote{teletypewriter - Elektromehanička pisaća mašina uparena sa računarom putem komunikacionog kanala (korišćena kao U/I uređaj 50-ih godina)}) računaru zamenio do tada korišcjeni način (\texttt{paketne obrada}) koji je podrazumevao unošenje programa putem bušenih kartica. Pored unošenja naredbi, komandna linija nam pruža mogućnost ispisa teksta koji je rezultat rada programa i još pun toga.

Kada govorimo o savremenim operativnim sistemima koji pružaju visok stepen udobnosti krajnjem korisniku, komandna linija se dosta ređe koristi jer mnogobrojne zadatke možemo obaviti putem grafičkog \texttt{shell-a} tj. \texttt{GUI-a} --- grafičkog korisničkog interfejsa. Najčešće se komandna linija u okviru \texttt{GUI-a}  emulira putem programa koji se zove terminal emulator.

\subsection{Komandna linija u Linux okruženju}

Specijalni programi koji nam obezbeđuju emulaciju komandne linije u \texttt{Linux} okruženju nazivaju se: \texttt{terminal emulator} i tekstualni \texttt{shell}. Ova kombinacija predstavlja komandnu liniju.

\vspace{4mm} 

\begin{figure}[h]
	\centering
	\includegraphics[width=.6\textwidth]{term_emulator.png} % size, path
	\caption{Terminal emulator + bash shell program}
	\label{terbash}
\end{figure}

\vspace{2mm} 

U daljem tekstu kada budemo govorili o konzoli, terminalu, komandnoj liniji, shell-u i slično referisaćemo baš na pomenutu kombinaciju koja nam predstavlja potreban alat za rešavanje određenih zadataka.

Komande terminala mogu biti implementirane direktno u \texttt{shell} programu, a  najčešće su zasebni programi koji se nalaze na nekoj od lokacija definisanim u \texttt{PATH} varijabli, te programe pokreće \texttt{shell} program i prosleđuje im listu argumenata. U daljem tekstu kada kažemo komanda, mislićemo najčešće na neki program koji data komanda pokreće.

\newpage

\subsection{Komande shell-a} 

Date su često korišćene i korisne komande \texttt{shell-a}:

% TODO -- Ovo se mora preurediti :)

\begin{itemize}
	\item{\texttt{history} --- Štampa istoriju izvršavanih komandi}
	\item{\texttt{info imekomande} --- Štampa informacije o komandi terminala}
	\item{\texttt{pwd} --- Štampa trenutni radni direktorijum}
	\item{\texttt{ls} --- Izlistava sadržaj direktorijuma}
	\item{\texttt{cat} --- kopira standardni ulaz na standardni izlaz}
	\item{\texttt{wc} --- Broji reči}
	\item{\texttt{head imefajla} --- Štampa prvih 10 linija fajla}
	\item{\texttt{tail imefajla} --- Štampa poslednih 10 linija fajla}
	\item{\texttt{sort} --- Sortira}
	\item{\texttt{find} --- Pretražuje direktorijum}
	\item{\texttt{locate} --- Efikasna pretraga fajla u sistemu}
	\item{\texttt{grep} --- Traži patern u prosleđenim fajlovima}
	\item{\texttt{echo} --- Štampa string koji je prosleđen.}
\end{itemize}

\subsection{Man strane}

U \texttt{man} stranama možemo naći dokumentaciju za određene servise operativnih sistema, komandi \texttt{shell-a}  raznih programa, i slično.

\begin{enumerate}[1)]
		 \item{Opšte naredbe}
		 \item{Sistemski pozivi}
		 \item{Funkcije C standardne biblioteke}
		 \item{Specijalni fajlovi, drajveri}
		 \item{Formati datoteka}
		 \item{Igre i screen-saveri}
		 \item{Razno}
		 \item{Komande za administraciju sistemom}
\end{enumerate}

Na primer da bismo otišli na \texttt{man} stranu za sistemski poziv \texttt{open}, a sistemski pozivi se nalaze u sekciji 2 u terminalu bismo ukucali komandu: 

\begin{center}\texttt{man 2 open}\end{center}

\item{Možemo pretražiti sve man strane komandom: \\ \vspace{-3mm} \begin{center}\texttt{man -k . | grep KLJUCNA\_REC}\end{center}}


% TODO -- Sekcije ispod napisati
% \subsection{MAKE alat}
% 
% \subsection{GDB - GNU Debuger}
...
\section{Sistemski pozivi}

\subsection{Šta su sistemski pozivi}

Sistemski pozivi predstavljaju interfejs kojim možemo raditi kritične operacije na sistemu (npr. upisivanje u fajl, komuniciranje između procesa i sl.). Sistemski pozivi govore operativnom sistemu (tačnije jezgru) koji zadatak da obavi za nas.  

Na kursu operativnih sistema ćemo koristiti sistemske pozive koji su ustvari funkcije u programskom jeziku \texttt{C} koje nam obezbeđuju određenu funkcionalnost.

\begin{figure}[h]
	\centering
	\includegraphics[width=.6\textwidth]{syscalls.jpg} % size, path
	\caption{Sistemski pozivi u hijerarhiji OS-a}
	\label{fig:syscalls}
\end{figure} 

Na slici \ref{fig:syscalls} možemo videti mesto sistemskih poziva u strukturalnoj hijerarhiji operativnog sistema.

% TODO -- Zavrsiti sekciju o sistemskim pozivima

\newpage

\subsection{Rad sa fajlovima}

\syscall{Open}{Sistemski poziv}
{int open(const char *pathname, int flags); \\
int open(const char *pathname, int flags, mode\_t mode);
}{Prvi argument funkcije je lokacija fajla koji želimo da otvorimo, drugi argument je kombinacija flagova: 

\texttt{O\_RDONLY} --- Otvara fajl samo sa čitanje\\
\texttt{O\_WRONLY} --- Otvara fajl samo za pisanje\\
\texttt{O\_RDWR} --- Otvara fajl za čitanje/pisanje\\
\texttt{O\_APPEND} --- Otvara fajl i postavlja offset na kraj fajla\\
\texttt{O\_CREAT} --- Kreira fajl ukoliko ne postoji\\
\texttt{O\_EXCL} --- U kombinaciji sa \texttt{O\_CREAT} zahteva da fajl bude kreiran \\
\texttt{O\_TRUNC} --- Postavlja veličinu fajla na 0 prilikom otvaranja 

Treći argument je opcioni, ukoliko ga navedemo \texttt{open()} će pokušati da otvori fajl sa navedenim pravima pristupa koja se zadaju oktalno (npr. 0644, 0777) itd. više informacija o pravima pristupa u poglavlju koji se bavi ovom tematikom.
}{\texttt{Fajl deskriptor} u slučaju uspeha, \texttt{-1} u slučaju greške i \texttt{errno} promenljiva se postavlja na odgovarajuću vrednost greške.}

\syscall{Creat}{Sistemski poziv}
{int creat(const char *path, modei\_t mode); $\Leftrightarrow$ \\ open(path, O\_WRONLY|O\_CREAT|O\_TRUNC, mode);}
{\texttt{creat()} sistemski poziv odgovara datom \texttt{open()} sisteskom pozivu, nedostatak mu je što otvara fajl samo za čitanje.}
{\texttt{Fajl deskriptor} u slučaju uspeha, -1 u slučaju greške i \texttt{errno} promenljiva se postavlja na odgovarajuću vrednost.}

\syscall{Close}{Sistemski poziv}
{int close(int fd);}
{Zatvara fajl na koji referiše fajl deskriptor}
{\texttt{0} u slučaju uspeha, -1 u slučaju greške.}

\syscall{Read}{Sistemski poziv}
{ssize\_t read(int fd, void *buf, size\_t count);}
{Pokušava da pročita count bajtova u bafer buf}
{\texttt{broj pročitanih bajtova (0 označava kraj fajla)}, -1 u slučaju greške i \texttt{errno} se postavlja na odgovarajuću vrednost.}
...
\begin{itemize}
	\item{Možemo pretražiti sve man strane komandom: \\ \vspace{-3mm} \begin{center}\texttt{man -k . | grep KLJUCNA\_REC}\end{center}}
			\vspace{1mm} 
	\item{Možemo pretražiti man strane sistemskih poziva komandom: \\ \vspace{-3mm} \begin{center}\texttt{man -2 syscalls | grep KLJUCNA\_REC}\end{center}}
			\vspace{1mm} 
